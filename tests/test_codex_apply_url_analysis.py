...
def test_select_analysis_targets_reads_job_posts_from_database(tmp_path) -> None:
    ...
    targets = select_analysis_targets(db_path, source="inthiswork", limit=1)
...
def test_build_codex_command_uses_xhigh_fast_defaults(tmp_path) -> None:
...
def test_run_apply_url_analysis_dry_run_does_not_call_codex(tmp_path) -> None:
    db_path = tmp_path / "employment.sqlite"
    _seed_job_post(db_path)
    calls: list[tuple[Sequence[str], str]] = []
    ...
    results = run_apply_url_analysis(
        ...
        run_command=lambda command, prompt: calls.append((command, prompt)),
    )

    assert len(results) == 1
    assert results[0].status == "planned"
    assert not results[0].prompt_path.exists()
    assert calls == []
...
def test_run_apply_url_analysis_skips_existing_output(tmp_path) -> None:
    ...
    first_results = run_apply_url_analysis(
        CodexApplyUrlAnalysisOptions(
            db_path=db_path,
            output_dir=tmp_path,
            dry_run=True,
    ...
    first_results[0].output_path.parent.mkdir(parents=True, exist_ok=True)
    first_results[0].output_path.write_text("existing\n", encoding="utf-8")
